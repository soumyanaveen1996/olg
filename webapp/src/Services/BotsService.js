...
function parseInputMessage(data) {
	...
	if (messageType === MessageTypeConstants.MESSAGE_TYPE_FORM2) {
		...
		message.aiccPlayer(data.message);
	} else if (
...
const sendMessage = async (message) => {
	...
	if (botContext?.botId === "im-bot") {
		if (message?.type === "chatInWindow") {
			const data =
				store.getState()?.chats?.message?.conversation?.allowNonContactMessages;
		...
		saveDataInLFStorage(`${LFStorageKeys.MESSAGES}_${conversationId}`, [
			message,
			...chatMessages,
		]);
	}
	let prevMessages = botContext.conversationContext
		? await getDataFromLFStorage(conversationId)
		: [];
...
export const sendSocketReconnectionEvent = () => {
	...
	const Message = botContext.getCapability("Message");
	let message = new Message({
		messageType: "onSocketReconnection",
		messageTypeInt: 170,
	});
