...
function SattracklandSigninView(props) {
	...
	useEffect(() => {
		if (
			...
			if (location.pathname === THURAYA_T2M_SIGNUP_LANDING) {
				setSignupPath(THURAYA_T2M_LANDING);
			} else {
		...
		if (query) {
			...
			if (state.corporateSignup) {
				activateEnterpriseBotAndSetLastLoggedInDomain(
					R.prop("sattracklandDomain", Config)
				)
					.catch((error) => {
...
export default SattracklandSigninView;
